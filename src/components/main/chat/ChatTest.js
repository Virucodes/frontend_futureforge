          ...
          <div className="flex flex-col gap-4 overflow-y-auto mb-10 p-4 bg-white rounded-lg shadow-md" ref={chatContainerRef}>
            {!chat && (
              <div className="flex justify-start">
                <div className="rounded-lg p-3 bg-[#0077B6] text-white font-medium shadow-md w-2/3">{messageTest.initial}</div>
              </div>
            )}
              ...
              <div className="flex flex-col gap-4 w-full">
                {chat.message.map((message, index) => (
                  <div key={index}>
                    {/* AI Message */}
                    <div className="flex justify-start">
                      <div className="p-3 bg-[#0077B6] text-white rounded-md shadow-md w-2/3">{message}</div>
                    </div>
                    {/* User Response */}
                    {chat.response[index] && (
                      <div className="flex justify-end mt-3">
                        <div className="p-3 bg-[#CAF0F8] text-[#023E8A] rounded-md shadow-md w-2/3">{chat.response[index]}</div>
                      </div>
                    )}
                  </div>
                ))}
